export const TINY_BOOK_HEIGHT = 40;
export const TINY_BOOK_WIDTH = 240;
export const TINY_BOOK_BASE_LEFT = 3690;
export const TINY_BOOK_BASE_BOTTOM = 4300;

export const LARGE_BASE_BOOK_HEIGHT = 6;
export const LARGE_BASE_BOOK_WIDTH = 36;
export const LARGE_BOOK_BASE_LEFT = 1540;
export const LARGE_BOOK_BASE_BOTTOM = 4810;

export const LARGE_BOOK_PADDING_PART = 0.15;